...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const allPaths = users.map((user: any) => {
    return {
      params: {
        id: user.id
      }
...
export const getStaticProps: GetStaticProps<{data: UsersProps[]}> = async ({params}) => {
  const id = params?.id
  const usersData = await loadPosts()
  const users = usersData as UsersProps[]

  const userData = users.filter((user: any) => id === user.id)
  return {
    props: {
      data: userData,
    }
...
export default function UserDetails({data}: InferGetStaticPropsType<typeof getStaticProps>) {
